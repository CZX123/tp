...
6. **Enhance `event add` by allowing users to define the status**. Currently, `event add` adds events with a default status of PENDING. We plan to enhance event adding by allowing users to define the initial status as well, so users can add ONGOING or DONE events in one shot without executing `event edit` afterwards.
7. **Enhance `event export` by allowing exports for multiple participant status**. Currently, `event export` only allows exports for participants with one specific status from AVAILABLE, UNAVAILABLE and UNKNOWN. We plan to enhance event exporting by allowing users to define multiple statuses, or a special `--status all` option to export all participants regardless of status.